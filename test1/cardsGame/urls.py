...
urlpatterns = [
    ...
    url(r'^cardsGame/bundles/divide/$', bundle_view.BundleDivide.as_view()), #Create new bundle from the a list of concret elements
    ...
    url(r'^cardsGame/bundles/merge/(?P<bundle_source_id>[0-9]+)/(?P<bundle_destination_id>[0-9]+)/$', bundle_view.bundle_merge), #Put elemente of source bundle into destinations bundle
    url(r'^cardsGame/bundles/exchange/(?P<bundle_player1_id>[0-9]+)/(?P<bundle_player2_id>[0-9]+)/$', bundle_view.bundle_exchange), #Exchange players of the two bundles
